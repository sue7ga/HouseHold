...
sub get_total_day{
 ...
 $total_info->{good} = $itr->good || 0;
 $total_info->{fare} = $itr->fare || 0;
 $total_info->{society} = $itr->society || 0;
 $total_info->{entertainment} = $itr->entertainment || 0;
 $total_info->{teaching} = $itr->teaching || 0;
 $total_info->{dress} = $itr->dress || 0;
 $total_info->{clinic} = $itr->clinic || 0;
 $total_info->{communicate} = $itr->communicate || 0;
 $total_info->{water} = $itr->water || 0;
 $total_info->{living} = $itr->living || 0;
 $total_info->{car} = $itr->car || 0;
 $total_info->{tax} = $itr->tax || 0;
 $total_info->{large_consume} = $itr->large_consume || 0;
 $total_info->{other} = $itr->other || 0; 
 return $total_info;
}
